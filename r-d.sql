...
insert into cities (name,country,population,area) values ('Tokyo','Japan',3850500,8223);
insert into cities (name,country,population,area) values ('Delhi','India',28125000,2240);
insert into cities (name,country,population,area) values ('Sao paulo','Brazil',209355000,3043);
insert into cities (name,country,population,area) values ('Shanghai','China',22125000,4015);

select name,population ,area from cities c 
select name,population ,area ,population/area as population_density from cities c 
select name ||', '|| country  as location from cities c
select concat(name,', ' ,country) as location from cities c

select name from cities where area > 4000; 
select name from cities where area = 8223; 
select name from cities where area <> 8223;
select name from cities where area between  2000 and 4000;
select name from cities where name in ('Delhi','Shanghai');
select name from cities where name not in ('Delhi','Shanghai');
select name from cities where area not in (3043, 8223) and name='Delhi';
select name from cities where area not in (3043, 8223) or name='Delhi';


select name,population ,area ,population/area as population_density from cities c where population/area > 6000;
...
insert  into photos (url,user_id) values('http://img1.jpg',8);
insert  into photos (url,user_id) values('http://img2.jpg',7);
insert  into photos (url,user_id) values('http://img3.jpg',6);
insert  into photos (url,user_id) values('http://img4.jpg',5);


select * from photos
select * from photos where user_id=8;
select  * from photos join users on users.id = photos.user_id;

--- Null constraints requirements
...
insert  into photos (url,user_id) values('http://img2.jpg',7);
insert  into photos (url,user_id) values('http://img3.jpg',6);
insert  into photos (url,user_id) values('http://img4.jpg',7);
...
select contents,username from "comments" c join users u on c.user_id =u.id;
select contents,url from "comments" c  join photos p on p.id=c.photo_id ;

select c.id as comment_id, p.id as photo_id
from photos p 
...
select contents,url from "comments" c join photos p on c.photo_id = p.id where c.user_id = p.user_id 

----three way join
select contents,url,username from "comments" c 
join photos p on c.photo_id = p.id 
join users u on u.id=c.user_id and u.id=p.user_id 
where c.user_id = p.user_id 

---------Grouping and aggregation
select user_id from "comments" c group by c.user_id ;

select max(id) from "comments" c ;
select min(id) from "comments" c ;
select avg(id) from "comments" c ;
select count(id) from "comments" c ;
select sum(id) from "comments" c ;

select user_id,max(id) from "comments" c group by c.user_id ;
select user_id,count(id) as num_comments_created from "comments" c group by c.user_id ;

select count(user_id) from photos;
----null values not included in counts
...
select * from products order by price limit 5 offset 1; 
...
----sub queries inside the where clause
...
select name,department,price from products p1 where p1.price = (select  max(price) from products as p2 where p2.department = p1.department);

select name,(select count(*) from orders o where p.id = o.product_id) from products p;
...
-----Utilit operators,keywords and functions
select name,weight,greatest(30, 2* weight) from products;

select name,price, least(price*0.5,300) from products;
...
alter table products alter column price set default 9999;
...
alter table orders add check(created_at<est_delivery);
